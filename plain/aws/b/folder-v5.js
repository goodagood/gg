...
var Promise, assert, async, bucket, build_new_folder, build_new_folder_and_save, check_delete_file, check_pattern, delete_all_file, delete_file, delete_path_uuid, get_file_meta, get_file_meta_by_path, get_file_meta_by_uuid, get_file_uuid, get_folder_uuid, get_sorted_message_list_as_ul, is_folder_exists, make_folder_meta_file_s3key, make_promisified_s3folder, make_s3folder, myconfig, myutil, new_folder, old, p, path, peek_possible_exist_folder, promised_make_s3folder, retrieve_file_by_path_uuid, retrieve_file_meta, retrieve_file_meta_pu, retrieve_file_objs, retrieve_files_by_pattern, retrieve_first_file_obj, retrieve_folder, retrieve_folder_meta, retrieve_metas_by_pattern, retrieve_promisified_folder, s3file, s3file_type, stop, test_add_folder, test_get_file_meta_by_path, test_get_file_meta_by_uuid, test_init_home_folder_many, test_read_folder_meta, test_retrieve_a_css, u, _file_name_to_delete_, _test_delete_member_file, _test_folder_path;
...
make_s3folder = function(folder_path) {
  var add_file, add_file_save_folder, add_folder, build_file_list, calculate_s3_meta_file_path, check_username, clear_empty_names, del_uuid_and_name, delete_all_uuid_recursively, delete_file, delete_file_by_uuid, delete_folder, delete_name, delete_uuid, delete_uuid_without_save, file_exists, get_file_name_array, get_file_obj_by_uuid, get_file_objs, get_file_objs_by_name, get_folder, get_member_manager, get_meta, get_number_of_name, get_one_file_obj, get_owner_name, get_ul_renderring, get_uuids, init, is_owner, list_files, list_files_and_save, list_files_for_owner, list_files_for_viewer, members, old_get_file_objs_by_name, pattern_to_uuids, prepare_basic_path, promise_to_add_file_save_folder, promise_to_clear_empty_names, promise_to_delete_file_by_uuid, promise_to_delete_name, promise_to_give_obj, promise_to_list_files_and_save, promise_to_one_file_obj, promise_to_retrieve_saved_meta, promise_to_save_meta, read_file, read_file_12_15, read_file_by_uuid, read_files, read_text, retrieve_saved_meta, save_meta, self_render_as_a_file, sort_files_by_date, uuid_to_delete_file_fun, uuid_to_file_obj, uuid_to_meta, write_text_file, _add_extra_0929, _add_file_obj_save_folder, _add_file_thorough, _add_file_to_redis_list, _build_blueimp_pic_gallery_list, _check_in_file, _d_read_file, _delete_file, _dot_filter, _empty_promise, _folder_, _folder_css_file_name_, _get_files_by_name, _get_owner_obj, _get_renderring_for_public, _get_renderring_for_viewer, _get_short_json_repr, _give_ul_renderring, _list_files_by_id, _member_, _meta_, _meta_css_file_name_, _meta_smells, _new_json_file, _prepare_redis_file_list_key, _ready_, _rename_file, _render_all_files, _render_file, _render_folder, _short_clone_of_folder_meta, _sleep;
  _folder_ = {};
  _meta_ = {};
  _member_ = null;
  _ready_ = 0;
  ...
  calculate_s3_meta_file_path = function(path_) {
  ...
  is_owner = function(username) {
    ...
    guess_owner = _meta_.path.split("/")[0];
    if (username === guess_owner) {
      return true;
    }
    return false;
  };
  ...
  _give_ul_renderring = function(viewer_name, callback) {
    ...
    return _has_member(viewer_name, function(is_member) {
      if (is_member) {
        return callback(get_ul_renderring());
      }
      return _has_viewer(viewer_name, function(is_viewer) {
        if (is_viewer) {
  ...
  sort_files_by_date = function() {
    var sorted;
    sorted = u.sortBy(_meta_.files, function(e) {
      var date, epoc;
  ...
  uuid_to_file_obj = function(uuid, callback) {
    ...
    if (file_meta != null) {
      ...
      return callback('not such uuid', null);
    }
  ...
  delete_name = function(name, callback) {
    ...
    return async.map(uuids, delete_file_by_uuid, callback);
  };
  ...
  _folder_.calculate_s3_meta_file_path = calculate_s3_meta_file_path;
...
retrieve_metas_by_pattern = function(folder_path, pattern, callback) {
  return retrieve_files_by_pattern(folder_path, pattern, function(err, file_list) {
    return async.map(file_list, function(file_obj) {
      return file_obj.get_meta();
    }, callback);
  });
};
...
is_folder_exists = function(path_, callback) {
  ...
  return make_s3folder(path_).then(function(folder) {
    Folder = folder;
    folder.calculate_s3_meta_file_path(path_);
