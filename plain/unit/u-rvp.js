...
var vp = require("../aws/plugin-ft/what-video-file-p.js");
...
function c_0818(){
    ...
    get_file.get_1st_file_obj_by_path(file_path, function(err, file){
        ...
        //p('meta path: ', m.path);
        p('c 0818 meta path: ', fp);
        //p('meta path uuid: ', m.path_uuid);
        p('c 0818 meta path uuid: ', pu);

        rvp.render_video_page_pu(pu, function(err, html){
            p(' -- 0818 1250 you  got: ', err, html.slice(0,300));
            stop();
        });
...
function c_0819(){
    ...
    get_file.get_1st_file_obj_by_path(file_path, function(err, file){
        ...
        rvp.render_video_page(fp, function(err, html){
            p(' -- 0819 you  got: ', err, html.slice(0,300));
            //fs.writeFile('/home/ubuntu/workspace/muji/plain/static/tmp/urvp.html', html, function(err, wres){});
            fs.writeFile('/tmp/vt.html', html, function(err, wres){


                p('write file: err, result: ', err, wres);
                stop();
            });
        });
...
function sample_meta_920(){
    ...
    get_file.get_1st_file_obj_by_path(file_path, function(err, file){
        ...
        fs.writeFile('/home/za/tmp/v-meta.json', JSON.stringify(meta, null, 4), function(e,r){
            p('wrote'); p(e,r);
            process.exit();
        });
    });
