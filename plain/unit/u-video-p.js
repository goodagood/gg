...
function check_918a(){
    ...
    get_file.get_1st_file_obj_by_path(file_path, function(err, file){
        if(err){ p('check 918a got err: ', err); return process.exit(); }

        var meta = file.get_meta();

        var checked = '';
            if(meta.unique) checked = 'checked';
        ...
        var context   = {
                ...
                username:  meta.owner,
                puuid:     meta.path_uuid,
                unique_is_checked : checked,
        };

        //p('meta: ', meta);
        p('context: ', context);
        p('meta.path: ', meta.path);


        vp.render_video_web_page(meta, function(err, html){
            if(err) p('vp r v w p, got err: ', err);
            //p('r f w p: ', err, html);
            fs.writeFile('/tmp/v.html', html, 'utf-8', function(e, r){
                if(e) p('write tmp html err: ', e);
